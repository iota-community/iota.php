...
class converter {
   ...
   * @return string
   */
  static public function bin2hex(string $val): string {
    return \bin2hex($val);
  }
   ...
   */
  static public function hex2bin(string $val): string|false {
    if(!self::isHex($val)) {
      throw new \iota\exception\converter("Input string must be hexadecimal string");
    }
   ...
   * @throws exception\converter
   */
  static public function bits(array $data, int $inLen, int $fromBits, int $toBits, $_pad = true):array {
    $_acc    = 0;
    $_bits   = 0;
    $_ret    = [];
    $_maxv   = (1 << $toBits) - 1;
    ...
    if($_pad) {
      ...
      throw new \iota\exception\converter('Invalid data');
    }
