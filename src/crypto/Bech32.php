...
class Bech32 {
  ...
  static public function decode(string $sBech): array {
    $_len = \strlen($sBech);
    if($_len < 8) {
      throw new \Exception("Bech32 is too short");
    }
    $_chars  = \array_values(\iota\converter::string2ByteArray($sBech));
    $_hUpper = false;
    $_hLower = false;
    $_pos    = -1;
    for($_i = 0; $_i < $_len; $_i++) {
    ...
    if($_pos === -1) {
      throw new \Exception("No separator character");
    }
    if($_pos < 1) {
      throw new \Exception("HRP is empty");
    }
    if(($_pos + 7) > $_len) {
      throw new \Exception('Checksum is to short');
    }
